...
function Nametags:__init()
    --Большинство настроек, которые вы хотите настроить, находятся здесь:
    self.enabled            = true
    ...
    self.zero_health        = Color( 255,  78, 69 ) -- Нулевой цвет здоровья
    self.full_health        = Color( 55,  204, 73 ) -- Полный цвет здоровья

    self.size               = TextSize.Default -- Размер шрифта
...
function Nametags:CreateSettings()
    ...
    self.window:SetTitle( "Настройки Тегов" )
    ...
    enabled_checkbox:GetLabel():SetText( "Использовать" )
    ...
    vehicle_checkbox:GetLabel():SetText( "Теги транспорта" )
    ...
    minimap_checkbox:GetLabel():SetText( "Иконки игроков на миникарте" )
    ...
    player_text:SetText( "Дистанция игроков (м)" )
    ...
    vehicle_text:SetText( "Дистанция транспорта (м)" )
    ...
    Events:Fire( "HelpAddItem",
        {
            name = "Теги",
            text = 
                "Теги - это имена, которые вы видите над игроками и транспортом." ..
                "\n\n" ..
                "Чтобы настроить их, введите /tags в чате и откроется " ..
                "окно, в котором вы можете выбрать свои собственные настройки."
        } )
end
