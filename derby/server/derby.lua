...
function Derby:ModuleUnload()
    ...
    for k,v in pairs(self.players) do
        v:Leave()
        self:MessagePlayer(v.player, "Вы были восстановлены на своем стартовом поле.")
    end
    self.players = {}
end
...
function Derby:PostTick()
    if ( os.difftime(os.time(), self.last_broadcast) >= 5*60 ) then
        self:MessageGlobal( "Дерби продолжается! Введите /derby, чтобы войти." )
    ...
    for k,v in pairs(self.players) do
        if v.timer ~= nil then
            if v.timer:GetSeconds() > 20 then
                ...
                self:MessagePlayer( v.player, "Вы были убиты за то, что не были в автомобиле!" )
            end
        end
    end
end
...
function Derby:MessagePlayer(player, message)
    player:SendChatMessage( "[Дерби] " .. message, Color(0xfff0b010) )
end

function Derby:MessageGlobal(message)
    Chat:Broadcast( "[Дерби] " .. message, Color(0xfff0c5b0) )
end

function Derby:EnterDerby(player)
    --[[if player:GetWorldId() ~= -1 then
        self:MessagePlayer(player, "Перед присоединением вы должны выйти из всех других игровых режимов.")
        return
    end]]--
    ...
    self:MessagePlayer(player, "Вы вошли в дерби! Введи /derby чтобы выйти.") 
...
function Derby:LeaveDerby(player)
    ...
    self:MessagePlayer(player, "Вы покинули дерби! Введи /derby, чтобы войти в любое время.")    
...
function Derby:PlayerSpawn(args)
    ...
    self:MessagePlayer(args.player, 
        "Вы появились на дерби. Введите /derby, если вы хотите выйти.")
...
function Derby:PlayerExitVehicle(args)
    ...
    if args.player:GetHealth() > 0.1 then
        ...
        self:MessagePlayer(args.player, 
            "Если вы не сядите в атомобиль в течение 20 секунд, вы будете убиты.")
